// backend/src/chain/pradaToken.js
...
async function lockTokens(userAddress, amountPrada) {
  const prada = getPradaContract();
  const amt   = ethers.parseUnits(amountPrada.toString(), 18);
  const tx    = await prada.lockTokens(userAddress, amt);
  await tx.wait();
  return tx.hash;
}

async function unlockTokens(userAddress, amountPrada) {
  const prada = getPradaContract();
  const amt   = ethers.parseUnits(amountPrada.toString(), 18);
...
async function getAccountInfo(userAddress) {
  const prada = getPradaContract();
  const [total, locked, unlocked, isLocked] = await prada.getAccountInfo(userAddress);
  return {
...
module.exports = { lockTokens, unlockTokens, getAccountInfo };
